...
// @name        CTSm (Tinychat)
// @version     1.0.50
// @description CTSm (CTS V1.8.54 modified)
// @author      CosmosisT, Osti
...
(function() {
    ...
    var CFG = {
        hiddenUsers: [], //Not yet implemented
        ...
        alwaysBot: false, //Ensure only one person has this enabled else it'll case reoccuring !bot spam every 2 minutes
    };

    //*************************************
    //******* OWNER PROXY COMMANDS ********

    //if a message begins with '/'
    function processCommand(msg, user){
        //if(!CTS.Me.owner) return;
        //if(!CTS.UserList[user].mod) return;
        if(!CTS.SuperModList.includes(CTS.UserList[user].username.toUpperCase())) return;
        ...
        var username = s[1].toUpperCase();
        if(username === "" || !CheckUserNameStrict(username)) Alert(GetActiveChat(), "Command Rejected");
        var handle = UsernameToHandle(username);
        if(handle == -1) Alert(GetActiveChat(), "Username not found");
        Alert(GetActiveChat(), "Closing " + username + " handle: " + handle);
        switch(s[0]){
            case '/closemod':
                Send("stream_moder_close", handle);
                // CTS.SocketTarget.send(JSON.stringify({
                //     "tc": "stream_close",
                //     "handle": handle
                // }));
    ...
    function isNumeric(str){
        ...
        if(!isNaN(threshold) && str === '' + threshold) return threshold;
    }
    function checkStringFormat(str, form){
        //form = "-n-s=SURGE-n";
        var arr = form.split("-"); //n s=surge n
        var tmp = str + '';
        tmp = tmp.toLowerCase();
        for(var i = 0; i < arr.length; i++){
            if(arr[i].startsWith("s")){
                var s = arr[i].substring(2, arr[i].length);
                console.log("s=" + s);
                if(tmp.startsWith(s)) {
                    tmp = tmp.substring(s.length, tmp.length);
                }
                else return false;
            }
            if(arr[i].startsWith("n")){
                ...
                for(var ii=0; ii < tmp.length; ii++){
                    if (is_numeric_char(tmp[ii])){
                        n += tmp[ii];
                    }
                    else break;
                }
                if (n == "") return false;
                else tmp = tmp.substring(n.length, tmp.length);
                console.log("n=" + n);
            }
            console.log("tmp = " + tmp);
        }
    ...
    function checkFatal(){
     ...
     var closedByMod = false;
     var broadcastReadyAttempts = 0;
     function StartBroadcasting(){
        broadcastReadyAttempts = 0;
        setTimeout(_broadcastReady, 300);
     }
     function _broadcastReady(){
         if(closedByMod) { closedByMod = false; return; }
        var e = VideoListElement.querySelector('#videos-footer-broadcast');
        if(e.innerText == 'Start Broadcast') setTimeout(function() { VideoListElement.querySelector("#videos-footer-broadcast").click(); }, 1000);
        broadcastReadyAttempts++;
        if(broadcastReadyAttempts > 100) return;
        setTimeout(_broadcastReady, 1000);
     }
    ...
    console.error = function(){
        if(arguments[0] != null){
            if(arguments[0].toString().includes("Broadcast closed due server request") && CTS.ReCam){
                StartBroadcasting();
            }
        }
    ...
    function onStart(){
        ...
        setTimeout(function(){
            ...
            if(CTS.FirstLoad){
                alert("This is a crudely modified version of the cosmosisT script\n\nIt was originally intended for personal use within a single room, therefore there may be unforeseen bugs.\n\nTake special care with the ReCam function as this simulates a click on the broadcast button which, if bugged, could result in an unintentional broadcast.\n\nIf you cannot read and understand the code that has been added so that you can trust this for yourself, I'd urge you not to use this script.");
                Alert(GetActiveChat(), "<b><u>CAUTION</u>:</> This script may contain bugs\n\n<u><b>Use at your own discretion</b></u>\n\nUnderstand that the ReCam function will simulate clicks on the start broadcast button which may result in an unexpected cam up if there's a bug\n\nI'd urge you to read and understand the ReCam code should you choose to use this script");
                Save("FirstLoad", false);
            }

            var currentDiv = VideoListElement.querySelector("#button-banlist");
            currentDiv.style.backgroundColor = "transparent";//"#35363A";
            currentDiv.style.margin = "0px 10px 0px 0px";

            currentDiv.innerText = "🚫";
            currentDiv.style.fontSize = "20px";
            VideoListElement.querySelector("#videos-header").appendChild(currentDiv);

            var reCamButton = document.createElement("button");
            reCamButton.setAttribute("id", "button-reCam");
            reCamButton.style.cssText = "display:block;width:50px;margin:0px 20px;";
            reCamButton.style.borderColor = "transparent";
            reCamButton.style.color = "white";
            reCamButton.style.fontSize = "12px";
            console.log("ReCam state: " + CTS.ReCam);
            if(CTS.ReCam){
                reCamButton.style.backgroundColor = "#013e7a";
                Alert(GetActiveChat(), "<b><u>CAUTION</u>:</b> ReCam enabled\n\nThis will attempt immediate reconnect if your cam is closed by the server\n\nFor safety, while this is enabled please assume your cam could start at any time");
            }
    ...
    var CTS = {
        ...
        Favorited: ["champchat", null, null, null, null],
        SafeList: [],
        GreenRoomIgnoreList: [],
        GreenRoomList: [],
        WatchList: [],
    ...
    function CTSRoomInject() {
        if(window.CFG == null) {
            console.log("**************************************");
            console.log("CTSm ERROR: Failed to load CTSm config");
            console.log("**************************************");
            //Backup config if alt script isn't loaded
            var CFG = {
                trustedMods: ["DAVID23", "JIMCROW", "LATTE", "DEEE", "SHYLOCKJU", "MMK25", "KIKID", "OBLIVIOUSOSTRICH"],
                greetPeople: true,
                alwaysGreetPeople: false,
                greetDelay: "16000",
                greetCooldown: 1200000,
                greetWbTime: 7200000,
                followupChance: 0,

                customGreetings: {"USERNAME": "nickname"},
                Welcomes: ["Hey", "Hi"],
                Followups: ["What's up?", "How're you?"],
                WelcomeBacks: ["Welcome back", "wb"],
        ...
        window.CTSEightBall = ["Stop fucking asking me questions!", "It is certain.", "It is decidedly so.", "Without a doubt.", "Yes - definitely.", "You may rely on it.", "As  I see it, yes.", "Most Likely.", "Outlook good.", "Yes.", "Signs point to yes.", "Reply hazy, try again.", "Ask again later.", "Better not tell you now.", "Cannot predict now.", "Concentrate and ask again.", "Don't count on it.", "My reply is no.", "My sources say no.", "Not a chance", "Hell no", "Definitely not", "Nooope", "NO!", "Outlook not so good.", "Very doubtful."];
    ...
    function BotCommandCheckPUB() {
        if (arguments[1].match(/^!whoisbot$/i)) {
            var t = Date.now();
            if(t - window.lastBotCheck > CTS.BotCheckCooldown) {
    ...
    function BotCheck() {
        if (CTS.UserList[arguments[0]].mod) {
            //CHECK HOST
            if (arguments[1] == ("!bot")) { //arguments[1].match(/^!bot$/i)
                //SET HOST
    ...
    function Alert() {
        CTS.Message[arguments[0]].push({
            ...
            "avatar": (arguments[2] !== undefined) ? ("") : ("https://avatars.tinychat.com/2c/1725a8/ef/medium/phpSZ6Tuo.gif"),
            "username": "",
            "nick": (arguments[2] !== undefined) ? (arguments[2]) : ("CTSm v: " + Ver()),
    ...
    function AddUserNotification() {
        if (CTS.FullLoad && CTS.ShowedSettings) {
            ...
            for(var i = 0; i < CFG.hiddenUsers.length; i++){
                if(arguments[3].toUpperCase() == CFG.hiddenUsers[i]){
                    return;
                }
            }
            if (arguments[0] == 1) {
    ...
    function checkWildcards(user){
        for(var i = 0; i < CTS.KickWildList.length; i++){
            if(checkStringFormat(user, CTS.KickWildList[i])) return true;
        }
        return false;
    }
    function CheckUserAbuse() {
        var action = false;
        if (CTS.Me.mod) {
            if (CTS.UserKickList.includes(arguments[1]) || CTS.NickKickList.includes(arguments[2].toUpperCase()) || checkWildcards(arguments[1].toLowerCase())) {
                CTS.NoGreet = true;
    ...
    var CommandList = {
        ...
        ctsm: function(){
            Alert(GetActiveChat(), "!botCooldown [milliseconds]\n!userKickWild [format] [example username]\nstring match: -s=something\nnumber match: -n\nEG: -n-s=username-n 1username4526\n\n!userKickWildList\n!userKickWildRemove\n!toggleReCamAllBrowsers\n!alwaysGreet\n!checkForBot\n!toggleGifts\n!hidecaps\n!hideCaps [Threshold]\n!hide420");
        },
        supermodadd: function(){
            if(arguments[0] === "" || !CheckUserNameStrict(arguments[0])) Alert(GetActiveChat(), "Command Rejected");
            else if (!CTS.SuperModList.includes[arguments[0].toUpperCase()]){
                CTS.SuperModList.push(arguments[0].toUpperCase());
                Save("SuperModList", JSON.stringify(CTS.SuperModList));
                Alert(GetActiveChat(), "Command Accepted");
            }
        },
        supermodremove: function(){
            if(arguments[0] === "" || !CheckUserNameStrict(arguments[0])) Alert(GetActiveChat(), "Command Rejected");
            else {
                var index = CTS.SuperModList.indexOf(arguments[0].toUpperCase());
                if (CTS.SuperModList[index] !== undefined){
                    CTS.SuperModList.splice(index, 1);
                    Save("SuperModList", JSON.stringify(CTS.SuperModList));
                    Alert(GetActiveChat(), "Command Accepted");
                }
                else Alert(GetActiveChat(), "Username not found");
            }
        },
        supermodlist: function(){
            Alert(GetActiveChat(), SettingsList.SuperModList());
        },
        ...
        checkforbot: function(){
            CTS.CheckForBot = !CTS.CheckForBot;
            Save("CheckForBot", CTS.CheckForBot);
            Alert(GetActiveChat(), CTS.CheckForBot ? "!whoisbot check enabled" : "!whoisbot check disabled");
        },
        alwaysgreet: function(){
            CTS.AlwaysGreet = !CTS.AlwaysGreet;
            Save("AlwaysGreet", CTS.AlwaysGreet);
            Alert(GetActiveChat(), CTS.AlwaysGreet ? "AlwaysGreet enabled" : "AlwaysGreet disabled");
        },
        botcooldown: function(){
            var delay = parseInt(arguments[0]);
            if(!isNaN(delay) && arguments[0] === '' + delay) {
                CTS.HideCaps = true;
                CTS.BotCheckCooldown = delay;
                Save("BotCheckCooldown", CTS.BotCheckCooldown);
                Alert(GetActiveChat(), "Setting cooldown to " + delay);
            }
            else Alert(GetActiveChat(), "Command Rejected");
        },
        firstload: function(){
            CTS.FirstLoad = true;
        ...
        userkickwild: function(){
            //arguments[0] = arguments[0];
            var arr = arguments[0].split(' ');
            if(!checkStringFormat(arr[1], arr[0]) || arguments[0] === ""){
                Alert(GetActiveChat(), "Command Rejected");
                return;
            }
            CTS.KickWildList.push(arr[0]);
            Save("KickWildList", JSON.stringify(CTS.KickWildList));
            Alert(GetActiveChat(), "Command Accepted");
        },
        userkickwildlist: function(){
            Alert(GetActiveChat(), SettingsList.UserKickWildList());
        },
        userkickwildremove: function(){
            //arguments[0] = arguments[0].toLowerCase();
            if (arguments[0] === "" || isNaN(arguments[0]) || CTS.KickWildList[arguments[0]] == undefined){
                Alert(GetActiveChat(), "Command Rejected");
                return;
            }
            CTS.KickWildList.splice(arguments[0], 1);
            Save("KickWildList", JSON.stringify(CTS.KickWildList));
            Alert(GetActiveChat(), "Command Accepted");
        },
        hide420: function(){
    ...
    var SettingsList = {
        ...
        SuperModList: function(){
            var index,
                msg,
                len = CTS.SuperModList.length;
            msg = "<b style=\"color:#ffffff;\"><u>SuperMod list:</u></b>\n" + ((!len) ? "empty\n" : "");
            for (index = 0; index < len; index++) msg += index + " : " + CTS.SuperModList[index] + "\n";
            return msg;
        },
        UserKickWildList: function(){
            var index,
                msg,
                len = CTS.KickWildList.length;
            msg = "<b style=\"color:#ffffff;\"><u>User Kick Wild list:</u></b>\n" + ((!len) ? "empty\n" : "");
            for (index = 0; index < len; index++) msg += index + " : " + CTS.KickWildList[index] + "\n";
            return msg;
        },
        UserBanList: function() {
            var index,
    ...
    var ServerInList = {
        ...
        join: function() {
            ...
            setTimeout(() => {
                ...
                var name = customName != "n/a" ? customName : cleanNickname(arguments[0].nick);
                var wb = false;
                ...
                //skip guests
                if(arguments[0].nick.toLowerCase().substring(0, 5) == "guest"){
                ...
                if ((CTS.Host == CTS.Me.handle || CTS.AlwaysGreet) && arguments[0].greet == true) {
                    ...
                    console.log("MSG = " + msg);
                    if(window.CFG.greetPeople && arguments[0].greet) Send("msg", msg);
            ...
            }, window.CFG.greetDelay, arguments);
        ...
        sysmsg: function() {
            var a = arguments[0].text.match(/^([a-z0-9_]{1,32}):? (closed|banned|kicked) ([a-z0-9_]{1,32})$/i);
            if(a[2] == "closed"){
                console.log("USER WAS CLOSED");
                var u = NicknameToUser(a[3].toUpperCase());
                if(CTS.UserList[u].handle == CTS.Me.handle){
                ...
                if (user != -1) {
                    if (!LineSpam(arguments[0].text, CTS.UserList[user].mod)) {
                        if (GamePrevention(arguments[0].text, CTS.UserList[user].mod)) {
                            ...
                            // * change *
                            var username = CTS.UserList[user].username.toUpperCase();
                            ...
                            for(var i = 0; i < CFG.hiddenUsers.length; i++){
                                if(CFG.hiddenUsers[i] == username) return;
                            }
    ...
    function SetLocalValues() {
        if (CTS.StorageSupport) {
            ...
            CTS.BotCheckCooldown = JSON.parse(Load("BotCheckCooldown", JSON.stringify(0)));
            CTS.AlwaysGreet = JSON.parse(Load("AlwaysGreet", JSON.stringify(false)));
            CTS.CheckForBot = JSON.parse(Load("CheckForBot", JSON.stringify(true)));
            CTS.KickWildList = JSON.parse(Load("KickWildList", JSON.stringify([])));
            CTS.SuperModList = JSON.parse(Load("SuperModList", JSON.stringify([])));
            CTS.Chat = JSON.parse(Load("Chat", JSON.stringify([])));
